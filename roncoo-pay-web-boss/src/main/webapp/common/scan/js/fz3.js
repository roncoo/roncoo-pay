...
//数据源来自cookie，展示部分信息
//获取最近一次输入个人信息（银行卡号）
function getPrivateInfo2(){
	alert("getPrivateInfo2");
	var bankCode = getCookie('bankCode');
	var bankAccountName = getCookie('bankAccountName');
	var certNo = getCookie('certNo');
	var bankAccountNo = getCookie('bankAccountNo');
	...
	//不接受残缺cookie
	if(bankCode!=null && bankAccountName!=null && certNo!=null && bankAccountNo!=null && phoneNo!=null && info!=null){
		var s=$("#bankAccountName").val();
		$("#bankAccountName").val("js插入");
		alert("info:"+info);
		//赋展示值
//		$("#bankCode").val(bankCode);
		$("#bankAccountName2").val(bankAccountName);
		$("#certNo2").val(certNo);
		$("#bankAccountNo2").val(bankAccountNo);
		$("#phoneNo2").val(phoneNo);
	}
}
